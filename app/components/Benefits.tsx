    ...
    <div className="bg-surface-muted border-b border-border">
      <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-y lg:divide-y-0 divide-slate-100">
        {items.map((item, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2 sm:gap-3 px-2.5 sm:px-6 py-2 sm:py-4">
              <div className="flex h-7 w-7 sm:h-10 sm:w-10 items-center justify-center rounded-lg sm:rounded-xl bg-brand-yellowSft text-navy flex-shrink-0 [&_svg]:h-3.5 [&_svg]:w-3.5 sm:[&_svg]:h-[18px] sm:[&_svg]:w-[18px]">
                <Icon size={18} aria-hidden="true" />
              </div>
              <div className="min-w-0">
                <p className="text-[11px] sm:text-[13px] font-semibold text-navy tracking-tight leading-tight">{item.title}</p>
