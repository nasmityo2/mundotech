...
describe('checkoutSchema — paymentUploadToken (Sesión 05 CORREGIDO)', () => {
  beforeEach(() => {
  ...
  // ── Token ausente ──────────────────────────────────────────────
  it('exige token de upload para métodos manuales', () => {
    ...
    if (!parsed.success) {
      ...
      expect(messages).toContain('Sube el comprobante de pago.');
    }
  });
  ...
  // ── Token vacío ────────────────────────────────────────────────
  it('rechaza token vacío para método manual', () => {
    const parsed = checkoutSchema.safeParse(
    ...
    if (!parsed.success) {
      const messages = parsed.error.issues.map((i) => i.message);
      // El schema Zod tiene min(1) que agrega su propio mensaje, y el superRefine
      // agrega otro. Verificamos que al menos uno de ellos mencione el comprobante.
      expect(
        messages.some(
          (m) =>
            m.includes('comprobante') ||
            m.includes('Token de subida requerido'),
        ),
      ).toBe(true);
    }
  });
