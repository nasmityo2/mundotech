...
export function googleMapsBusinessUrl(): string {
  const custom = process.env.NEXT_PUBLIC_GOOGLE_MAPS_BUSINESS_URL?.trim();
  if (custom) return custom;
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(GOOGLE_MAPS_LOCATION_QUERY)}`;
}
...
export function googleMapsEmbedUrl(): string {
  ...
  if (custom) return custom;
  const q = encodeURIComponent(GOOGLE_MAPS_LOCATION_QUERY);
