...
| JSON-LD producto/sitio | `ProductJsonLd.tsx`, JSON-LD en `layout.tsx` | 1 CSP (PRD-284) — política dual jun 2026 |
...
| [x] doc | PRD-241, PRD-264 — comportamiento aceptado documentado; PRD-284 — CSP dual ISR ✅ (jun 2026) |
...
| DEPENDENCIA-01 / H19 | ✅ Cerrado | CSP dual pública + JSON-LD ISR sin nonce (`middleware.ts`, `JsonLd.tsx`, PRD-284, jun 2026) |
...
| Observabilidad | 5/10 | Sentry opt-in + `global-error.tsx`; falta DSN en prod; al activar Sentry, ampliar `connect-src` en ambas ramas CSP |
...
| PRD-010 | 🟠 ✅ | Variables R2 validadas al arranque | `lib/env-validation.ts` |
...
| **Headers seguridad** | HSTS, X-Frame-Options, CSP dual en `middleware.ts` (pública cacheada `unsafe-inline` / dinámica nonce+`strict-dynamic`) |
...
- [ ] **Banners y promociones** activos, URLs R2 válidas
