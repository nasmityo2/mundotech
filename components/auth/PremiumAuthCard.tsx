...
export default function PremiumAuthCard({
  ...
  useEffect(() => {
    const id = window.requestAnimationFrame(() => {
      if (tab === 'login') document.getElementById('auth-email')?.focus();
  ...
  return (
    <motion.section
      ...
      className={cn(
        ...
        'bg-[#111827]/88 backdrop-blur-2xl shadow-[0_24px_80px_-24px_rgba(0,0,0,0.85)]',
        'overflow-hidden flex flex-col max-h-[min(92dvh,780px)]',
      )}
    >
      ...
      <div className="relative flex items-start justify-between gap-3 p-6 md:p-8 pb-4 md:pb-5 border-b border-white/10">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <h1 id="auth-page-title" className="mt-1 text-lg md:text-xl font-bold tracking-tight text-white">
              Acceso a tu cuenta
            </h1>
            <p className="mt-1 text-[13px] text-white/50 leading-snug">
              Compra con garantía · historial de pedidos · favoritos sincronizados
            </p>
          </div>
        </div>
        <Link
